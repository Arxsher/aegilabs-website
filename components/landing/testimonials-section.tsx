...
const testimonials = [
  {
    quote: "An agent can answer common customer questions, summarize context, and escalate only the conversations that need a human.",
    author: "Support Agent",
    role: "Use case",
    company: "Customer care",
    metric: { value: "24/7", label: "First response" },
  },
  {
    quote: "An agent can qualify inbound leads, collect missing details, update the CRM, and prepare the next follow-up.",
    author: "Sales Agent",
    role: "Use case",
    company: "Lead handling",
    metric: { value: "1", label: "Clean CRM flow" },
  },
  {
    quote: "An agent can turn scattered updates into a clear weekly report for managers, teams, and clients.",
    author: "Reporting Agent",
    role: "Use case",
    company: "Operations",
    metric: { value: "1", label: "Weekly report" },
  },
  {
    quote: "An agent can check order status, send confirmations, and notify the team when an exception needs attention.",
    author: "Operations Agent",
    role: "Use case",
    company: "Order follow-up",
...
const asciiBackground = Array.from({ length: 60 }, (_, row) =>
  Array.from({ length: 100 }, (_, col) => {
    const noise = (row * 37 + col * 19 + row * col * 7) % 100;
    return noise > 70 ? '"' : " ";
  }).join("")
).join("\n");
        ...
        <div className="flex items-center justify-between mb-20">
          <div>
            ...
            <h2 className={`text-4xl lg:text-5xl font-display transition-all duration-1000 ${
              ...
              Use cases that
              <span className="text-background/40"> repeat weekly.</span>
            </h2>
          </div>
