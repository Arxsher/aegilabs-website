...
const systemLayers = [
  { name: "Workflow map", nodes: "audit", status: "ready" },
  { name: "Agent runtime", nodes: "managed", status: "live" },
  { name: "Tool connectors", nodes: "integrated", status: "synced" },
  { name: "Monitoring", nodes: "reviewed", status: "active" },
];
...
export function InfrastructureSection() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveRegion((prev) => (prev + 1) % systemLayers.length);
    }, 3000);
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                AegiLabs runs the agent layer for you: workflow logic, tool access,
                monitoring, approvals, and continuous improvement.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              ...
              <p className="text-muted-foreground max-w-md">
                A simple operating system for agents: mapped workflows, connected tools, supervised execution, and measurable outputs.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">Human</span>
              <span className="block text-sm text-muted-foreground mt-2">Approval when needed</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">Logs</span>
              <span className="block text-sm text-muted-foreground mt-2">Every action reviewable</span>
            </div>
          </div>
        </div>
